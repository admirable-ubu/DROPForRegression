 ...
 * Establece los métodos comunes para los algoritmos de la familia DROP.
 * </p>
 * 
 * @author Álvar Arnaiz González
 * @version 1.2
 */
public abstract class DROPAlgorithm extends Algorithm implements Serializable {
	 ...
	 */
	public DROPAlgorithm (Instances train) throws NotEnoughInstancesException {
		super(train);
	 ...
	 * @throws NotEnoughInstancesException Si el dataset no tiene instancias.
	 */
	public DROPAlgorithm (Instances train, int [] inputDatasetIndex) throws NotEnoughInstancesException {
	 ...
	 */
	protected int calcWith (Instances set) {
		...
		for (Instance assoc : mAssociates.elementAt(mCurrInstancePos)) {
			...
			neighbours = new Vector<Instance>(mNeighbours.elementAt(pos).size() - 1);
			...
			// vecino.
			if (!isMisclassified(assoc, neighbours))
				with++;
		}
	 ...
	 */
	protected int calcWithout (Instances set) {
		Vector<Instance> neighbours;
		int pos, without = 0;
		...
		for (Instance assoc : mAssociates.elementAt(mCurrInstancePos)) {
			...
			// Copiar la lista de vecinos sin incluir a currInstance.
			neighbours = new Vector<Instance>(mNeighbours.elementAt(pos).size() - 1);
